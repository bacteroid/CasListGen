...
if os.path.isfile(configpat):
    ...
    ## Set Variables
    baset["title"] = config['CAST']['title']
    baset["source"] = config['CAST']['source']
    baset["cover"] = config['CAST']['cover']
    baset["logo"] = config['CAST']['logo']
    baset["ext"] = config['CAST']['ext']
    try:
        srset["port"] = int(config['SERVER']['port'])
        srset["cycle"] = int(config['SERVER']['cycle'])
        srset["mode"] = config.getboolean('SERVER', 'mode')
    except:
...
def givmepag(baset):
    ...
    * {
    ...
    return {"Head":head,"Foot":foot,"Player":playerpag,"Style":style}
    

## Generate card for each episode
def gencrd(title,idn,baset):
...
def stgen(baset=baset):
    ## Default Icon / Cover Generator for Your Cast.
    def genicon(fnam,source):
    ...
    ## Generating Page
    # {"Head":head,"Foot":foot,"Player":playerpag,"Style":style}
    blocks = givmepag(baset)
    page = blocks["Head"]+seljs+menu+genlst+blocks["Foot"]
...
def castgen(srset=srset,baset=baset):
    t = threading.currentThread()
    while getattr(t, "run", True):
        flst = [f for f in os.listdir(baset['source']) if os.path.isdir(os.path.join(baset['source'],f))]
        if not srset["flstbuf"]:
            srset["flstbuf"] = flst
        else:
            comp = [f for f in flst if f not in srset["flstbuf"]]
            if comp:
                stgen()
                print("+===============+")
            else:
...
if srset["mode"]:
    ...
    Protocol = "HTTP/1.0"
    server_address = ('127.0.0.1', srset["port"])
    HandlerClass.protocol_version = Protocol
    httpd = ServerClass(server_address, HandlerClass)
    sa = httpd.socket.getsockname()

    cst = threading.Thread(target = castgen)
    cst.start()
